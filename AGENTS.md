...
- 终端 UI 可以独立于 Python Agent Core 演进。Python 负责 Agent、LLM、工具执行；React + Ink 前端负责终端界面、输入体验和消息状态。
- 前后端通信优先采用 JSON Lines 事件协议，避免 UI 直接依赖具体 Agent 内部实现。
...
- 关键函数顶部使用简短中文 docstring，说明函数用途、主要参数含义和返回值。
- 复杂逻辑前使用 1 到 3 行中文注释，优先解释为什么这样写。
- 不注释简单语句，例如 import、变量赋值、简单 if、简单 return。
- 不重复解释代码表面含义。
- 注释必须保持简洁，不改变代码逻辑。
- 核心流程可以适当多写，辅助函数只写一句说明。
